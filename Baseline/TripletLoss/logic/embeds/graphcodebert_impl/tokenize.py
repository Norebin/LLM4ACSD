...
for lang in dfg_function:
    # lang_file = pathlib.Path(__file__).parent.resolve() / 'parser' / 'my-languages.so'
    # language_lib = Language.load_library(str(lang_file))
    # lang_ptr = language_lib.get_language(lang)
    LANGUAGE = Language('/model/lxj/Baseline/TripletLoss/logic/embeds/graphcodebert_impl/parser/my-languages.so', lang)
    parser = Parser()
...
def build_dataset(file_path):
    ...
    tokenizer = RobertaTokenizer.from_pretrained("/model/LiangXJ/Model/microsoft/graphcodebert-base")
