...
class FullModel(nn.Module):
    def __init__(self, vocablen, metriclen, hidden, d_model, metric_list_len, nhead, num_gcn_layers, num_encoder_layers, dim_feedforward, dropout, alpha):
        ...
        self.W2 = nn.Parameter(torch.empty(size=(metric_list_len*hidden, hidden)).to('cuda'))
    ...
    def forward(self, x, edge_index, edge_attr, metrics):
        ...
        out1 = self.gcn(x, edge_index).reshape(1,-1)
...
class myDNN(nn.Module):
    def __init__(self, hidden, metric_list_len):
        super(myDNN, self).__init__()
        hidden_size = hidden
        metriclen = metric_list_len
