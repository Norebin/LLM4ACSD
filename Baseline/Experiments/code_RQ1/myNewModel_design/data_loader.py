...
def get_x(nodes, vocabdict):
    ...
    for value in nodes.values():
        if value not in vocabdict:
            ...
            x.append(vocabdict[value])
    # for i in range(len(nodes)):
    #     #x.append(nodes[str(i)])
    #     if nodes.values()[str(i)] not in vocabdict:
    #         x.append(vocabdict['__Empty_Token__'])
    #         #print('keyErro')
    #     else:
...
def get_token_list(nodelist, vocabdict):
    token_list = []
    for value in nodelist.values():
        if value not in vocabdict:
            token_list.append(vocabdict['__Empty_Token__'])
        else:
            token_list.append(vocabdict[value])
    return token_list
    ...
    for jsonFolderPath in jsonFolderPathList:
        for root ,dirs, files in os.walk(jsonFolderPath):
            for file in files:
                ...
                token_list = get_token_list(data["nodes"], vocabdict)
    ...
    for jsonFolderPath in jsonFolderPathList:
        for root ,dirs, files in os.walk(jsonFolderPath):
            for file in files:
                ...
                for token in data['nodes'].values():
