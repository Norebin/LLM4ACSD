...
class FullModel(nn.Module):
    def __init__(self, vocablen, metriclen, hidden, d_model, nhead, num_gcn_layers, num_encoder_layers, dim_feedforward, dropout, alpha):
        ...
        self.W2 = nn.Parameter(torch.empty(size=(3*hidden, hidden)).to('cuda'))
...
class FullModel_DeepFusion(nn.Module):
    def __init__(self, vocablen, metriclen, hidden, d_model, nhead, num_gcn_layers, num_encoder_layers, dim_feedforward, dropout, alpha):
        ...
        self.fc_r = nn.Linear(3*hidden, hidden)
        ...
        self.W2 = nn.Parameter(torch.empty(size=(3*hidden, hidden)).to('cuda'))
        nn.init.xavier_uniform_(self.W2.data, gain=1.414)
        self.Wo = nn.Parameter(torch.empty(size=((3+3)*hidden, 2*hidden)).to('cuda'))
