...
datasetPath = '/model/lxj/ACSDetector/dataCollection/lxjDataset/'

def get_train_test_list(projectList):
    """
    input:项目名称列表
    format:list
    output:new_trainlist, new_testlist
    format:list[xxx.java    label]
    """
    trainlist = []
    testlist = []
...
def data_split(json_dir):
    train_list = []
    test_list = []
    for jsonFolderPath in json_dir:
...
def init_data_and_model(projectList, curModel):
    jsonFolderPathList = []
    for projectname in projectList:
        jsonFolderPathList.append(datasetPath+ projectname +'/rawjson')
    ...
    trainlist, testlist = data_split(jsonFolderPathList)
...
def test(testlist, model_index, allJsonDict, batch_size, curModel):
    with torch.no_grad():
        ...
        for i in Test_data_batches:
            #lable
            line_info = batches[i][0]
            jsonName = line_info+'.json'
            #data
            try:
                data = allJsonDict[jsonName]
                label = data['label']
                ...
                notFound += 1
            if metrics.shape != (49, 1):
                metrics = torch.cat((metrics, torch.zeros(49 - metrics.shape[0], 1, device='cuda', dtype=metrics.dtype)), dim=0)
            #predict
            if curModel == myDNN:
...
if __name__ == '__main__':
    ...
    projectList = [name for name in os.listdir("/model/lxj/ACSDetector/dataCollection/lxjDataset") if os.path.isdir(os.path.join("/model/lxj/ACSDetector/dataCollection/lxjDataset", name))]
    modelList = [myDNN, myBiLSTM, FullModel][2:]

    for curModel in modelList:
        print(curModel)
        test_result_path = 'stirling_result.txt'
        times = 3
        result_list = []
