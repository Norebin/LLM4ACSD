...
def create_graphml(nodes: List[str],
    ...
    # 保存为 GraphML 格式
    output_file = "/model/LiangXJ/DependencyFinder-1.4.3/" + output_name + ".graphml"
    nx.write_graphml(G, output_file)

    # 同时保存节点映射关系，方便后续使用
    output_json = "/model/LiangXJ/DependencyFinder-1.4.3/" + output_name + ".json"
...
def xml_to_graph(xml_file):
    ...
    # for package in tqdm(root.findall(".//package"), desc="Processing packages"):
        ...
        #     continue
    for class_node in tqdm(root.findall(".//class"), total=len(root.findall(".//class")), desc="Processing classes"):
...
if __name__ == "__main__":
    ...
    nodes, node_types, edge_index, edge_types, node_to_idx = xml_to_graph('/model/LiangXJ/DependencyFinder-1.4.3/test.xml')
    graph = create_graphml(nodes, node_types, edge_index, edge_types, node_to_idx, "calcu")
