...
from SmellDataset import SmellDataset
...
def calculate_metrics(y_true, y_pred):
    results = {
        'binary_precision': round(precision_score(y_true, y_pred, average='macro'), 4),
        'binary_recall': round(recall_score(y_true, y_pred, average='macro'), 4),
        'binary_f1': round(f1_score(y_true, y_pred, average='macro'), 4),
    }
    return results
...
def train(args, train_dataloader, valid_dataloader, test_dataloader, model):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch_data in enumerate(bar):
            ...
            # x = torch.nn.functional.pad(x, (0,49-x.size(1)))
            # if edge_index.numel() == 0:
            #     edge_index = torch.arange(x.size(0)).unsqueeze(0).repeat(2, 1).to(args.device)
            # edge_index, _ = add_remaining_self_loops(edge_index, num_nodes=x.shape[0])
            edge_index = handle_empty_graphs(batch_data)
...
def handle_empty_graphs(batch_data):
    x, edge_index, batch = batch_data.x, batch_data.edge_index.long(), batch_data.batch
    num_nodes = x.shape[0]

    if edge_index.nelement() == 0:
        # 使用 unique_consecutive 替代 bincount
        ...
        for num_nodes_in_graph in counts:
            if num_nodes_in_graph > 0:
                ...
                edge_indices.append(
                    torch.stack([node_indices, node_indices], dim=0)
                )
            start_idx += num_nodes_in_graph
...
def evaluate(args, eval_dataloader, model):
    ...
    with torch.no_grad():
        for step, batch_data in enumerate(eval_dataloader):
            batch_data.to(args.device)
            # graph:tensor(3680,768) (2,18862) (3680,); 添加自环边; 去除重复边
            x, edge_index, batch = batch_data.x, batch_data.edge_index.long(), batch_data.batch
            # x = torch.nn.functional.pad(x, (0, 49 - x.size(1)))
            # if edge_index.numel() == 0:
            #     edge_index = torch.arange(x.size(0)).unsqueeze(0).repeat(2, 1).to(args.device)
...
def main():
    ...
    parser.add_argument("--gnn_model", default="GCNConv", type=str,
                        help="GNN core.")
    parser.add_argument("--gnn_hidden_size", default=32, type=int,
                        help="hidden size of gnn.")
    parser.add_argument("--gnn_feature_dim_size", default=49, type=int,
                        help="feature dim size of gnn.4159")
    ...
    args.do_train = True
    args.do_test = False
    args.gnn_model = 'GCNConv'
    args.cuda_id = 3
    device = torch.device("cuda:" + str(args.cuda_id) if torch.cuda.is_available() else "cpu")
    args.device = device
    args.model_checkpoint_dir = '/model/LiangXJ/processed_graph_data/' + args.model_checkpoint_dir + '/' + args.gnn_model
    set_seed(args.seed)
    ...
    val_indices = test_val_indices
    train_dataloader = DataLoader([graph_dataset[i] for i in train_indices], batch_size=args.batch_size, shuffle=True, collate_fn=collate)
    valid_dataloader = DataLoader([graph_dataset[i] for i in val_indices], batch_size=args.batch_size, shuffle=False, collate_fn=collate)
    test_dataloader = DataLoader([graph_dataset[i] for i in test_val_indices], batch_size=args.batch_size, shuffle=False, collate_fn=collate)
    # train_dataloader = DataLoader(graph_dataset, batch_size=args.batch_size, shuffle=True, collate_fn=collate)
    print(graph_dataset)
