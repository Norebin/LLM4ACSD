...
# Cayenne contributes only its handed-over test partition. No Cayenne train
...
if missing_csv_ids:
    raise RuntimeError(
        f"Cayenne test features contain {len(missing_csv_ids)} IDs absent from CSV"
    )

manifest = {
    "experiment": "cayenne_downstream_holdout_reusing_handed_over_phi4",
    ...
    "seed": 42,
    "protocol_note": (
        "Cayenne is excluded from downstream fusion training. The handed-over Phi-4 "
