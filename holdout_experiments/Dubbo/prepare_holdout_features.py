...
# Dubbo contributes only its handed-over test partition. No Dubbo train
...
if missing_csv_ids:
    raise RuntimeError(
        f"Dubbo test features contain {len(missing_csv_ids)} IDs absent from CSV"
    )

manifest = {
    "experiment": "dubbo_downstream_holdout_reusing_handed_over_phi4",
    ...
    "seed": 42,
    "protocol_note": (
        "Dubbo is excluded from downstream fusion training. The handed-over Phi-4 "
