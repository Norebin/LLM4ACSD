...
from torch_geometric.nn import GINConv, SAGEConv, BatchNorm, global_mean_pool, global_max_pool
...
def load_semantic_features(file_path, feature_type):
    """从.pt文件中加载语义特征"""
    ...
    features = data.get('features', {}).get(feature_type, None)
    
    if features is None:
        raise ValueError(f"Feature type '{feature_type}' not found in file")
...
class FusionDataset(Dataset):
    ...
    def __init__(self, project_name, model_name, semantic_feature_dir, csv_dir, graph_root_dir,
                 split='train', semantic_feature_type=SEMANTIC_FEATURE_TYPE):
        """
        初始化融合数据集
        ...
        self.semantic_features, self.unique_ids = load_semantic_features(
            semantic_path, self.semantic_feature_type
        )
...
class GraphSAGEModel(nn.Module):
    ...
    def forward(self, data):
        ...
        x = F.dropout(F.relu(self.bn1(self.conv1(x, edge_index))),
                      p=DROPOUT_RATE, training=self.training)
        x = F.dropout(F.relu(self.bn2(self.conv2(x, edge_index))),
                      p=DROPOUT_RATE, training=self.training)
        x = F.dropout(F.relu(self.bn3(self.conv3(x, edge_index))),
                      p=DROPOUT_RATE, training=self.training)
...
class FusionClassifier(nn.Module):
    ...
    # 类变量，用于存储图节点特征的维度
    graph_input_dim = 78  # 默认值，将在运行时根据实际数据更新
    ...
    def __init__(self, semantic_dim, graph_dim, fusion_dim, fusion_type='self_attention',
        ...
        if gnn_type not in graph_encoders:
            raise ValueError(f"Unknown GNN type: {gnn_type}")
        encoder_class = graph_encoders[gnn_type]
        self.graph_encoder = encoder_class(
    ...
    def forward(self, semantic_feature, graph_data):
        ...
        graph_feature = self.graph_encoder(graph_data)
...
def main():
    ...
    parser.add_argument('--fusion_type', type=str, default='progress', 
                        help='特征融合方式')
    parser.add_argument('--semantic_feature_type', type=str, default=SEMANTIC_FEATURE_TYPE,
                        choices=['shallow_avg_pooling', 'middle_avg_pooling',
                                 'deep_avg_pooling', 'last_token'],
                        help='从语义特征PT文件中读取的特征类型')
    ...
    for project in projects:
        ...
        try:
            ...
            train_dataset = FusionDataset(
                ...
                split='train',
                semantic_feature_type=args.semantic_feature_type
            )
    ...
    FusionClassifier.graph_input_dim = node_feature_dim
    ...
    for fusion in fusion_type:
        ...
        print(f"Semantic feature type: {args.semantic_feature_type}, GNN type: {args.gnn_type}")
